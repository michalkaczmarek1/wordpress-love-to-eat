...
. "Content-Type: text/html; charset=\"utf-8\"\r\n";	
        ...
        for($i = 0; $i <= $count; $i++) {
            if ($_POST[$i] == 'checked') {
                wp_mail($message[$i]['field2'],$_POST['subject'],$body,$headers);
            }
        }
        qcf_admin_notice('Messages have been sent to selected names');
    }
    ...
    if (!$qppkey['authorised']) {
        ...
        $dashboard .= '<p>Subject:<br><input style="width:100%" type="text" name="subject" value=""/></p>';
        echo $dashboard;
        wp_editor('', 'message', $settings = array('textarea_rows' => '10','wpautop'=>false));
        $dashboard = '<p><input type="submit" name="qcf_email_selected" class="qcf-button" value="Send an email to selected names" /></p>';
        $dashboard .= wp_nonce_field("save_qcf");
        $dashboard .= '</form>';
    }
	echo $dashboard;
}
